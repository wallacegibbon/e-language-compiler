...
compile_to_c(InputFilename, OutputFilename) ->
	...
	%io:format(">> ~p~n~n", [Ast]),
	ecompiler_genc:generate_ccode(Ast, Vars, InitCode,
				      OutputFilename)
    catch
      {Filename, Errinfo} ->
...
parse_and_compile(Filename) ->
	...
	{Ast1, Vars, InitCode}
      ...
      E ->
	  ok = unrecord_compileop(Filename), throw({Filename, E})
    end.
...
record_module(Filename, FnMap) ->
    compilercd_cmd({record_module, filename_tomod(Filename),
		    FnMap}).
...
compilercd_loop(State) ->
    receive
	  ...
	  case compilercd_handle(Command, State) of
		...
		Pid ! {Ref, Result}, compilercd_loop(NewState);
	    stop -> Pid ! {Ref, ok}
	  end;
      Any -> throw(Any)
    end.
