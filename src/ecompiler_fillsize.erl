...
-export([expand_sizeof/2, expand_sizeof_inexprs/2,
	 fill_structinfo/2]).

-import(ecompiler_util,
	[cut_extra/2, exprsmap/2, fill_offset/2,
	 fillto_pointerwidth/2, flat_format/2, fn_struct_map/1,
	 getvalues_bykeys/2, names_of_varrefs/1,
	 primitive_size/1]).
...
expand_sizeof([#struct{field_defaults = FieldDefaults} =
		   S
	       | Rest],
	      Ctx) ->
    [S#struct{field_defaults =
		  expand_sizeof_inmap(FieldDefaults, Ctx)}
     | expand_sizeof(Rest, Ctx)];
expand_sizeof([], _) -> [].

expand_sizeof_inmap(Map, Ctx) ->
    maps:map(fun (_, V1) -> expand_sizeof_inexpr(V1, Ctx)
	     end,
	     Map).
...
offsetsof_struct(#struct{field_names = FieldNames,
    ...
    {_, OffsetMap} = sizeof_fields(FieldTypeList, 0, #{},
				   Ctx),
    OffsetMap.
