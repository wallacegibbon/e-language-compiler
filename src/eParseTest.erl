...
functionPointer_test() ->
    {ok, Tokens, _} = eScanner:string("a: fun(u8, u8^): u16^ = b;"),
    %?debugFmt("~p~n", [Tokens]),
...
functionCall_test() ->
    {ok, Tokens, _} = eScanner:string("b: u8 = a(13);"),
    %?debugFmt("~p~n", [Tokens]),
...
array_test() ->
    {ok, Tokens, _} = eScanner:string("a: {u8, 100};"),
    %?debugFmt("~p~n", [Tokens]),
...
arrayInit_test() ->
    {ok, Tokens, _} = eScanner:string("a: {u8, 2} = {11, 22};"),
    %?debugFmt("~p~n", [Tokens]),
...
structInit_test() ->
    {ok, Tokens, _} = eScanner:string("a: S = S{name=\"a\", value=2};"),
    %?debugFmt("~p~n", [Tokens]),
...
assign_test() ->
    {ok, Tokens, _} = eScanner:string("fun b() a * = 3;c bsr = 5;end"),
    %?debugFmt("~p~n", [Tokens]),
