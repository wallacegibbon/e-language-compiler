...
check_position(#e_expr{tag = '=', data = [_, Op2]}) when is_record(Op2, struct_init_expr); is_record(Op2, array_init_expr) ->
	ok;
...
array_init_to_ops(Target, [E | Rest], Cnt, Line, NewCode, StructMap) ->
	...
	B = [#e_expr{tag = '+', data = A, line = Line}],
	C = #e_expr{tag = '^', data = B, line = Line},
