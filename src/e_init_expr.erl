...
expand_in_function([#function{stmts = Exprs} = F | Rest], StructMap) ->
	...
	[F#function{stmts = expand_init_expr(Exprs, StructMap)}
		| expand_in_function(Rest, StructMap)];
...
expand_init_expr(
	[#if_stmt{then = Then, else = Else} = E | Rest],
	NewAST,
	StructMap
) ->
	expand_init_expr(
		Rest,
		[E#if_stmt{
			then = expand_init_expr(Then, [], StructMap),
...
default_value_of(#array_type{elem_type = Type, length = Len}, Line) ->
	#array_init_expr{
		elements = lists:duplicate(Len, default_value_of(Type, Line)),
		line = Line
	};
