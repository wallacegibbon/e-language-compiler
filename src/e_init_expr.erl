...
expand_init_expr([#if_stmt{then = Then, else = Else} = E | Rest], NewAST, StructMap) ->
    expand_init_expr(Rest, [E#if_stmt{then = expand_init_expr(Then, [], StructMap), else = expand_init_expr(Else, [], StructMap)} | NewAST],
                     StructMap);
...
replace_init_ops(#op2_expr{operator = assign, operand1 = Op1, operand2 = #array_init_expr{elements = Elements, line = Line}},
                 StructMap) ->
    array_init_to_ops(Op1, Elements, 0, Line, [], StructMap);
replace_init_ops(Any, _) ->
    [Any].

struct_init_to_ops(Target, [#var_ref{line = Line, name = Name} = Field | Rest],
