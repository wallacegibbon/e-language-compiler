...
expand_in_function([#function{stmts = Exprs} = F | Rest], StructMap) ->
    ...
    [F#function{stmts = expand_init_expr(Exprs, StructMap)} | expand_in_function(Rest,
                                                                                 StructMap)];
...
expand_init_expr([#if_stmt{then = Then, else = Else} = E | Rest], NewAST, StructMap) ->
    expand_init_expr(Rest,
                     [E#if_stmt{then = expand_init_expr(Then, [], StructMap),
                                else = expand_init_expr(Else, [], StructMap)}
                      | NewAST],
                     StructMap);
expand_init_expr([#while_stmt{stmts = Exprs} = E | Rest], NewAST, StructMap) ->
    expand_init_expr(Rest,
                     [E#while_stmt{stmts = expand_init_expr(Exprs, [], StructMap)} | NewAST],
                     StructMap);
...
struct_init_to_ops(Target,
                   [#var_ref{line = Line, name = Name} = Field | Rest],
                   FieldInitMap,
                   FieldTypes,
                   NewCode,
                   StructMap) ->
...
default_value_of(#array_type{elem_type = Type, length = Len}, Line) ->
    #array_init_expr{elements = lists:duplicate(Len, default_value_of(Type, Line)),
                     line = Line};
default_value_of(#basic_type{class = struct,
                             tag = Tag,
                             p_depth = 0},
                 Line) ->
    #struct_init_expr{name = Tag,
                      line = Line,
                      field_value_map = #{},
                      field_names = []};
