...
functionNormal_test() ->
    {ok, Tks, _} = ecompilerScan:string("fun a(v: i8^) v^ = 10; end"),
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks,
                 [{'fun', 1}, {identifier, 1, a}, {'(', 1}, {identifier, 1, v},
                  {':', 1}, {integerType, 1, i8}, {'^', 1}, {')', 1},
                  {identifier, 1, v}, {'^', 1}, {'=', 1}, {integer, 1, 10},
                  {';', 1}, {'end', 1}]),
    {ok, Ast} = ecompilerParse:parse(Tks),
    %?debugFmt("~p~n", [Ast]),
    ?assertEqual(Ast,
                 [{function_raw, 1, a,
...
functionPointer_test() ->
    {ok, Tks, _} = ecompilerScan:string("a: fun(u8, u8^): u16^ = b;"),
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks,
                 [{identifier, 1, a}, {':', 1}, {'fun', 1}, {'(', 1},
                  {integerType, 1, u8}, {',', 1}, {integerType, 1, u8},
                  {'^', 1}, {')', 1}, {':', 1}, {integerType, 1, u16},
...
functionCall_test() ->
    {ok, Tks, _} =
        ecompilerScan:string("b: u8 = a::b(13);"),
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks,
                 [{identifier, 1, b}, {':', 1}, {integerType, 1, u8},
                  {'=', 1}, {identifier, 1, a}, {'::', 1},
...
array_test() ->
    {ok, Tks, _} = ecompilerScan:string("a: {u8, 100};"),
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks,
                 [{identifier, 1, a}, {':', 1}, {'{', 1},
                  {integerType, 1, u8}, {',', 1}, {integer, 1, 100},
                  {'}', 1}, {';', 1}]),
    {ok, Ast} = ecompilerParse:parse(Tks),
    %?debugFmt("~p~n", [Ast]),
    ?assertEqual(Ast,
...
arrayInit_test() ->
    {ok, Tks, _} = ecompilerScan:string("a: {u8, 2} = {11, 22};"),
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks,
                 [{identifier, 1, a}, {':', 1}, {'{', 1},
                  {integerType, 1, u8}, {',', 1}, {integer, 1, 2}, {'}', 1},
                  {'=', 1}, {'{', 1}, {integer, 1, 11}, {',', 1},
                  {integer, 1, 22}, {'}', 1}, {';', 1}]),
    {ok, Ast} = ecompilerParse:parse(Tks),
    %?debugFmt("~p~n", [Ast]),
...
structInit_test() ->
    {ok, Tks, _} = ecompilerScan:string("a: S = S{name=\"a\", val=2};"),
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks,
...
assign_test() ->
    {ok, Tks, _} = ecompilerScan:string("fun b() a * = 3;c bsr = 5;end"),
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks,
