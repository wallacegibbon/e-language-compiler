...
-export([expr_to_str/1, expr_map/2, filter_var_refs_in_map/2, get_values_by_keys/2, fmt/2,
         ethrow/3, ethrow/2]).
...
expr_map(Fn, [#if_stmt{} = If | Rest]) ->
    [If#if_stmt{condi = Fn(If#if_stmt.condi),
                then = expr_map(Fn, If#if_stmt.then),
                else = expr_map(Fn, If#if_stmt.else)}
     | expr_map(Fn, Rest)];
...
expr_to_str(#if_stmt{condi = Cond,
                     then = Then,
                     else = Else}) ->
...
expr_to_str(#while_stmt{condi = Cond, stmts = Exprs}) ->
    io_lib:format("while (~s) ~s end",
                  [expr_to_str(Cond), lists:map(fun expr_to_str/1, Exprs)]);
...
make_function_and_struct_map_from_ast(AST) ->
    ...
    FnTypeMap =
        maps:from_list(
            lists:map(fun(#function{name = Name} = Fn) -> {Name, Fn#function.type} end, Fns)),
    StructMap =
        maps:from_list(
            lists:map(fun(#struct{name = Name} = S) -> {Name, S} end, Structs)),
    {FnTypeMap, StructMap}.
...
primitive_size_of(u64) ->
    8;
primitive_size_of(i64) ->
    8;
primitive_size_of(u32) ->
    4;
primitive_size_of(i32) ->
    4;
primitive_size_of(u16) ->
    2;
primitive_size_of(i16) ->
    2;
primitive_size_of(u8) ->
    1;
primitive_size_of(i8) ->
    1;
primitive_size_of(f64) ->
    8;
primitive_size_of(f32) ->
    4;
primitive_size_of(T) ->
    throw(fmt("size of ~p is not defined", [T])).

void_type(Line) ->
    #basic_type{class = void,
                tag = void,
                p_depth = 0,
                line = Line}.
