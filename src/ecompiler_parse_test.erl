...
function_normal_test() ->
    ...
    ?assertEqual(Tks, [{'fun',1}, {identifier,1,a}, {'(',1},
		       {identifier,1,v}, {':',1}, {integer_type,1,i8}, {'^',1},
		       {')',1}, {identifier,1,v}, {'^',1}, {'=',1},
		       {integer,1,10}, {';',1}, {'end',1}]),
...
function_pointer_test() ->
    ...
    ?assertEqual(Ast, [{vardef,1,a,
			{fun_type,1, [{basic_type,1,0,integer,u8},
...
function_call_test() ->
    {ok, Tks, _} = ecompiler_scan:string("b: u8 = a::b(13);"),
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks, [{identifier,1,b}, {':',1}, {integer_type,1,u8}, {'=',1},
		       {identifier,1,a}, {'::',1}, {identifier,1,b}, {'(',1},
		       {integer,1,13}, {')',1}, {';',1}]),
    {ok, Ast} = ecompiler_parse:parse(Tks),
    %?debugFmt("~p~n", [Ast]),
    ?assertEqual(Ast, [{vardef,1,b, {basic_type,1,0,integer,u8},
...
array_test() ->
    ...
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks, [{identifier,1,a}, {':',1}, {'{',1}, {integer_type,1,u8},
		       {',',1}, {integer,1,100}, {'}',1}, {';',1}]),
    {ok, Ast} = ecompiler_parse:parse(Tks),
    %?debugFmt("~p~n", [Ast]),
    ?assertEqual(Ast, [{vardef,1,a,{array_type,1,{basic_type,1,0,integer,u8},
				    {integer,1,100}},
			none}]),
    ok.
...
struct_init_test() ->
    ...
    {ok, Ast} = ecompiler_parse:parse(Tks),
    %?debugFmt("~p~n", [Ast]),
    ?assertEqual(Ast, [{vardef,1,a, {basic_type,1,0,struct,'S'},
			{struct_init_raw,1,'S',
...
assign_test() ->
    ...
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks, [{'fun',1}, {identifier,1,b}, {'(',1}, {')',1},
		       {identifier,1,a}, {'*',1}, {'=',1}, {integer,1,3},
		       {';',1}, {identifier,1,c}, {'bsr',1}, {'=',1},
		       {integer,1,5}, {';',1}, {'end',1}]),
    {ok, Ast} = ecompiler_parse:parse(Tks),
    %?debugFmt("~p~n", [Ast]),
    ?assertEqual(Ast, [{function_raw,1,b,[], {basic_type,1,0,void,void},
