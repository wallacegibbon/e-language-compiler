...
function_call_test() ->
    ...
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks, [{identifier,1,b}, {':',1}, {integer_type,1,u8},
		       {'=',1}, {identifier,1,a}, {'::',1},
		       {identifier,1,b}, {'(',1}, {integer,1,13}, {')',1},
		       {';',1}]),
    {ok, Ast} = ecompiler_parse:parse(Tks),
    %?debugFmt("~p~n", [Ast]),
...
array_test() ->
    ...
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks, [{identifier,1,a}, {':',1}, {'{',1},
		       {integer_type,1,u8}, {',',1}, {integer,1,100},
		       {'}',1}, {';',1}]),
    {ok, Ast} = ecompiler_parse:parse(Tks),
    %?debugFmt("~p~n", [Ast]),
    ?assertEqual(Ast, [{vardef,1,a,{array_type,1,
...
struct_init_test() ->
    ...
    %?debugFmt("~p~n", [Tks]),
    ?assertEqual(Tks, [{identifier,1,a}, {':',1}, {identifier,1,'S'},
		       {'=',1}, {identifier,1,'S'}, {'{',1},
