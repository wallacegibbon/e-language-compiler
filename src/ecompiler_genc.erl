...
fixfunction_for_c(#function{exprs = Exprs, var_types = VarTypes} = F, {FnMap, StructMap, GlobalVars}) ->
...
fixexprs_for_c(Exprs, Ctx) ->
    ecompiler_util:exprsmap(fun (E) -> fixexpr_for_c(E, Ctx) end, Exprs).

fixexpr_for_c(#op1{operator = '@', operand = Operand, line = Line} = E, {FnMap, StructMap, VarTypes} = Ctx) ->
    case ecompiler_type:typeof_expr(Operand, {VarTypes, FnMap, StructMap, none}) of
        #array_type{} ->
            #op2{operator = '.', op1 = fixexpr_for_c(Operand, Ctx), op2 = #varref{name = val, line = Line}};
        _ ->
            E
    end;
...
statements_tostr([#function{name = Name, param_names = ParamNames, type = Fntype, var_types = VarTypes, exprs = Exprs} | Rest], InitCode, StatementStrs, FnDeclars) ->
    ...
    PureParams = kvlist_frommap(ParamNameAtoms, maps:with(ParamNameAtoms, VarTypes)),
...
statements_tostr([#struct{name = Name, field_types = FieldTypes, field_names = FieldNames} | Rest], InitCode, StatementStrs, FnDeclars) ->
    FieldList = kvlist_frommap(get_names(FieldNames), FieldTypes),
    S = io_lib:format("struct ~s {~n~s~n};~n~n", [Name, listvars_to_str(FieldList)]),
...
params_to_str(NameTypePairs) ->
    lists:join(",", lists:map(fun ({N, T}) -> type_tostr(T, N) end, NameTypePairs)).
...
type_tostr(#array_type{len = Len, elemtype = ElementType}, Varname) ->
    io_lib:format("struct {~s val[~w];} ~s", [type_tostr(ElementType, ""), Len, Varname]);
type_tostr(#basic_type{class = Class, tag = Tag, pdepth = Depth}, Varname) when Depth > 0 ->
    io_lib:format("~s~s ~s", [typetag_tostr(Class, Tag), lists:duplicate(Depth, "*"), Varname]);
...
expr_tostr(#op2{operator = Operator, op1 = Op1, op2 = Op2}, Endchar) ->
    io_lib:format("(~s ~s ~s)~c", [expr_tostr(Op1, $\s), translate_operator(Operator), expr_tostr(Op2, $\s), Endchar]);
expr_tostr(#op1{operator = Operator, operand = Operand}, Endchar) ->
    io_lib:format("(~s ~s)~c", [translate_operator(Operator), expr_tostr(Operand, $\s), Endchar]);
