...
fetch_constants([#const{name = Name, val = Expr}
                 | Rest],
                Statements, Constants) ->
    fetch_constants(Rest,
                    Statements,
...
eval_constexpr(#op2{operator = '+', op1 = Op1,
                    op2 = Op2},
               Constants) ->
    eval_constexpr(Op1, Constants) +
        eval_constexpr(Op2, Constants);
eval_constexpr(#op2{operator = '-', op1 = Op1,
                    op2 = Op2},
               Constants) ->
    eval_constexpr(Op1, Constants) -
...
eval_constexpr(#op2{operator = '/', op1 = Op1,
    ...
    eval_constexpr(Op1, Constants) /
        eval_constexpr(Op2, Constants);
eval_constexpr(#op2{operator = 'rem', op1 = Op1,
                    op2 = Op2},
               Constants) ->
    eval_constexpr(Op1, Constants) rem
        eval_constexpr(Op2, Constants);
eval_constexpr(#op2{operator = 'and', op1 = Op1,
                    op2 = Op2},
               Constants) ->
    (eval_constexpr(Op1, Constants) =/= 0) and
        (eval_constexpr(Op2, Constants) =/= 0);
eval_constexpr(#op2{operator = 'or', op1 = Op1,
                    op2 = Op2},
               Constants) ->
    (eval_constexpr(Op1, Constants) =:= 1) or
        (eval_constexpr(Op2, Constants) =:= 1);
eval_constexpr(#op2{operator = 'band', op1 = Op1,
                    op2 = Op2},
               Constants) ->
    eval_constexpr(Op1, Constants) band
        eval_constexpr(Op2, Constants);
eval_constexpr(#op2{operator = 'bor', op1 = Op1,
                    op2 = Op2},
               Constants) ->
    eval_constexpr(Op1, Constants) bor
        eval_constexpr(Op2, Constants);
eval_constexpr(#op2{operator = 'bxor', op1 = Op1,
                    op2 = Op2},
               Constants) ->
    eval_constexpr(Op1, Constants) bxor
        eval_constexpr(Op2, Constants);
eval_constexpr(#op2{operator = 'bsr', op1 = Op1,
                    op2 = Op2},
               Constants) ->
    eval_constexpr(Op1, Constants) bsr
        eval_constexpr(Op2, Constants);
eval_constexpr(#op2{operator = 'bsl', op1 = Op1,
                    op2 = Op2},
               Constants) ->
    eval_constexpr(Op1, Constants) bsl
        eval_constexpr(Op2, Constants);
eval_constexpr(#varref{name = Name, line = Line},
               Constants) ->
...
replace_constants([#struct_raw{fields = Fields} = S
                   | Rest],
                  Constants) ->
    [S#struct_raw{fields =
                      replace_inexprs(Fields, Constants)}
     | replace_constants(Rest, Constants)];
replace_constants([#vardef{type = Type,
                           initval = Initval} =
                       V
                   | Rest],
                  Constants) ->
...
replace_inexpr(#varref{name = Name, line = Line} = Expr,
               Constants) ->
    case maps:find(Name, Constants) of
        {ok, Val} -> constnum_to_token(Val, Line);
        error -> Expr
    end;
