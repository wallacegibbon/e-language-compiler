...
handle_special(
	[{identifier, _, define}, {identifier, LineNumber, Name} | Rest],
...
handle_special(
	[{identifier, _, undef}, {identifier, LineNumber, Name} | Rest],
...
handle_special(
	[{identifier, _, ifdef}, {identifier, _, Name} | Rest],
...
handle_special(
	[{identifier, _, ifndef}, {identifier, _, Name} | Rest],
	{MacroMap, _, EndTag} = Ctx
) ->
	{MacroMapNew, CollectedTokens, RestTokensNew} =
		case MacroMap of
...
handle_special(
	[{identifier, _, endif} | RestContent],
	{MacroMap, RetTokens, endif}
) ->
	{MacroMap, RetTokens, RestContent};
...
handle_special(
	[{identifier, LineNumber, endif} | _] = Content,
	{MacroMap, RetTokens, else}
) ->
...
handle_special([{identifier, LineNumber, Name} | _], _) ->
	throw({
		LineNumber,
		e_util:fmt("unexpected operator \"~s\" here", [Name])
	});
...
handle_special([], {_, _, EndTag}) ->
	throw({
		0,
		e_util:fmt(
			"unexpected end of file while in state: \"#~s\"",
			[EndTag]
		)
	}).

-spec collect_to_else_and_ignore_to_endif([token()], context())
	-> handle_ret().
collect_to_else_and_ignore_to_endif(Tokens, {MacroMap, RetTokens, _}) ->
	%% collect "then" part
	{MacroMapNew, CollectedTokens, RestTokensRaw} =
		handle_normal(Tokens, {MacroMap, [], else}),
	%% ignore "else" part
	{_, _, RestTokens} =
		handle_normal(RestTokensRaw, {MacroMap, [], endif}),
...
handle_normal(
	[{'?', _}, {identifier, _, _} = Identifier | Rest],
	{MacroMap, _, _} = Ctx
) ->
	replace_macro(
		Identifier,
		MacroMap,
		fun (Tokens) -> handle_normal(Tokens ++ Rest, Ctx) end
	);
...
get_expr_till_eol(
	...
	replace_macro(
		...
		fun (Tokens) ->
			get_expr_till_eol(
				Rest,
				lists:reverse(Tokens) ++ CollectedTokens,
				Ctx
			)
		end
	);
...
replace_macro({identifier, LineNumber, Name}, MacroMap, ContinueHandler) ->
	case MacroMap of
		...
		_ ->
			throw({
				LineNumber,
				e_util:fmt("undefined macro ~s", [Name])
			})
	end.
...
replace_line_number(Tokens, LineNumber) ->
	lists:map(
		fun (Token) -> setelement(2, Token, LineNumber) end,
		Tokens
	).
...
process_no_operator_test() ->
	...
	?assertEqual(
		[{int_type, 1, u32}, {identifier, 1, a}, {'=', 1},
			{integer, 1, 1}, {';', 1}],
		process(Tokens)
	).
...
process_undef_2_test() ->
	{ok, Tokens, _} =
		e_scanner:string("#define A 1 + 2 + 3\n#undef B\n?A + 1"),
	?assertThrow(
		{2, "macro \"B\" is not defined"},
		tokens_to_str(process(Tokens))
	).
...
convert_elif_to_else_and_if(
	[{'#', _} = PreTag, {'if', _} = Token | Rest],
	_
) ->
	[PreTag, Token | convert_elif_to_else_and_if(Rest, 0)];
convert_elif_to_else_and_if(
	[{'#', _}, {elif, LineNumber} | Rest],
	ElifDepth
) ->
	[mk_elif_replacement(LineNumber)
		| convert_elif_to_else_and_if(Rest, ElifDepth + 1)];
convert_elif_to_else_and_if(
	[{'#', _} = PreTag, {identifier, _, endif} = Token | Rest],
	ElifDepth
) ->
	[lists:duplicate(ElifDepth + 1, [PreTag, Token])
		| convert_elif_to_else_and_if(Rest, 0)];
...
mk_elif_replacement(LineNumber) ->
	[
		{'#', LineNumber}, {else, LineNumber}, {'#', LineNumber},
		{'if', LineNumber}
	].
