...
handle_special([{identifier, _, undef}, {identifier, LineNumber, Name} | Rest], {MacroMap, RetTokens, EndTag}) ->
	case MacroMap of
	#{Name := _}	-> handle_normal(Rest, {maps:remove(Name, MacroMap), RetTokens, EndTag});
	_		-> e_util:ethrow(LineNumber, "macro \"~s\" is not defined", [Name])
	end;
handle_special([{identifier, _, ifdef}, {identifier, _, Name} | Rest], {MacroMap, _, EndTag} = Ctx) ->
	{MacroMapNew, CollectedTokens, RestTokensNew} =
		case MacroMap of
		#{Name := _}	-> collect_to_else_and_ignore_to_endif(Rest, Ctx);
		_		-> ignore_to_else_and_collect_to_endif(Rest, Ctx)
		end,
...
handle_special([{identifier, _, ifndef}, {identifier, _, Name} | Rest], {MacroMap, _, EndTag} = Ctx) ->
	{MacroMapNew, CollectedTokens, RestTokensNew} =
		case MacroMap of
		#{Name := _}	-> ignore_to_else_and_collect_to_endif(Rest, Ctx);
		_		-> collect_to_else_and_ignore_to_endif(Rest, Ctx)
		end,
	handle_normal(RestTokensNew, {MacroMapNew, CollectedTokens, EndTag});
...
handle_special([{'if', _} | Rest], {MacroMap, _, EndTag} = Ctx) ->
	...
	{MacroMapNew, CollectedTokens, RestTokensNew} =
		...
		true	-> collect_to_else_and_ignore_to_endif(RestTokens, Ctx);
		false	-> ignore_to_else_and_collect_to_endif(RestTokens, Ctx)
		end,
	handle_normal(RestTokensNew, {MacroMapNew, CollectedTokens, EndTag});
...
get_expr_till_eol(Tokens, Ctx) -> get_expr_till_eol(Tokens, [], Ctx).
...
replace_macro({identifier, LineNumber, Name}, MacroMap, ContinueHandler) ->
	case MacroMap of
	#{Name := Value}	-> ContinueHandler(replace_line_number(Value, LineNumber));
	_			-> e_util:ethrow(LineNumber, "undefined macro ~s", [Name])
	end.

%% tokens should be parsed to ast before evaluating them, this function will be updated when the parser is finished.
eval_token_exprs([{integer, _, 0}], _MacroMap) -> false;
eval_token_exprs([{integer, _, 1}], _MacroMap) -> true.
...
convert_elif_to_else_and_if_3_test() ->
	{ok, Tokens, _} = e_scanner:string("#if 0\n #else\n #endif"),
	?assertEqual("# if 0 \n # else \n # endif", tokens_to_str(convert_elif_to_else_and_if(Tokens))).

convert_elif_to_else_and_if_4_test() ->
	{ok, Tokens, _} = e_scanner:string("#if 0\n #if 1\n #else\n #endif #endif"),
	?assertEqual("# if 0 \n # if 1 \n # else \n # endif # endif", tokens_to_str(convert_elif_to_else_and_if(Tokens))).
