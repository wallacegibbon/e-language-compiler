...
fix_pointer_size(?OP2('^', O, ?I(0), Loc) = Op, TypeCtx, SizeCtx) ->
...
fix_pointer_size(#e_op{data = Operands} = Op, TypeCtx, SizeCtx) ->
	Op#e_op{data = lists:map(fun(E) -> fix_pointer_size(E, TypeCtx, SizeCtx) end, Operands)};
