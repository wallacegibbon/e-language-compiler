...
expandInitExpressions(Exprs, StructMap) -> prvExpandInitExpression(Exprs, [], StructMap).
...
prvExpandInitExpression([#while_expr{exprs = Exprs} = E | Rest], NewAst, StructMap) ->
    prvExpandInitExpression(Rest, [E#while_expr{exprs = prvExpandInitExpression(Exprs, [], StructMap)} | NewAst], StructMap);
...
prvReplaceInitOps(#op2{operator = assign, op1 = Op1, op2 = #struct_init{name = Name, line = Line, field_values = FieldValues}}, StructMap) ->
