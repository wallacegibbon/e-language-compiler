...
-spec check_types_in_ast(e_ast(), var_type_map(), {fn_type_map(), struct_type_map()}) ->
                          ok.
check_types_in_ast([#function{} = Fn | Rest],
                   GlobalVarTypes,
                   {FnTypeMap, StructMap} = Maps) ->
  #function{var_type_map = VarTypes,
            stmts = Stmts,
            type = FnType} =
    Fn,
  ...
  type_of_nodes(Stmts, {CurrentVars, FnTypeMap, StructMap, FnType#fn_type.ret}),
  check_types_in_ast(Rest, GlobalVarTypes, Maps);
check_types_in_ast([#struct{} = S | Rest],
                   GlobalVarTypes,
                   {FnTypeMap, StructMap} = Maps) ->
  #struct{name = Name,
...
-spec check_type_in_ast_nodes([e_stmt()],
                              var_type_map(),
...
type_of_node(#e_expr{tag = {call, FunExpr},
                     data = Args,
                     line = Line},
             Ctx) ->
...
type_of_node(#if_stmt{condi = Condi,
                      then = Then,
                      else = Else,
                      line = Line},
             Ctx) ->
...
type_of_node(#while_stmt{condi = Condi,
                         stmts = Stmts,
                         line = Line},
             Ctx) ->
  type_of_node(Condi, Ctx),
...
type_of_node(#goto_label{line = Line}, _) ->
  e_util:void_type(Line);
type_of_node(#type_convert{expr = Expr,
                           type = Type,
                           line = Line},
             Ctx) ->
...
check_struct_field(#var_ref{name = FieldName, line = Line},
  ...
  case compare_type(ExpectedType, GivenType) of
    ...
    false ->
      e_util:ethrow(Line,
                    "~s.~s type error: ~s = ~s",
                    [StructName, FieldName, type_to_str(ExpectedType), type_to_str(GivenType)])
  end.
