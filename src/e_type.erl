...
check_types_in_ast([#function{} = Fn | Rest], GlobalVarTypes, {FnTypeMap, StructMap} = Maps) ->
	#function{var_type_map = VarTypes, stmts = Stmts, type = FnType} = Fn,
...
check_types_in_ast([#struct{} = S | Rest], GlobalVarTypes, {FnTypeMap, StructMap} = Maps) ->
	#struct{name = Name, field_type_map = FieldTypes, field_names = FieldNames, field_default_value_map = FieldDefaults} = S,
	...
	check_types_in_struct_fields(InitFieldNames, FieldTypes, FieldDefaults, Name, {GlobalVarTypes, FnTypeMap, StructMap, #basic_type{}}),
	check_types_in_ast(Rest, GlobalVarTypes, Maps);
...
type_of_node(#e_expr{tag = '+', data = [Op1, Op2], line = Line}, Ctx) ->
...
type_of_node(#e_expr{tag = Tag, data = [Op1, Op2], line = Line}, Ctx)
  when Tag =:= '*'; Tag =:= '/' ->
	Op1Type = type_of_node(Op1, Ctx),
	Op2Type = type_of_node(Op2, Ctx),
	case are_both_number_of_same_type(Op1Type, Op2Type) of
...
type_of_node(#e_expr{tag = {call, FunExpr}, data = Args, line = Line}, Ctx) ->
...
type_of_node(#e_expr{tag = '^', data = [Operand], line = Line}, Ctx) ->
...
type_of_node(#e_expr{tag = '@', data = [Operand], line = Line}, {_, _, StructMap, _} = Ctx) ->
	case Operand of
...
check_struct_field(#var_ref{name = FieldName, line = Line}, FieldTypes, ValMap, StructName, Ctx) ->
	{_, _, StructMap, _} = Ctx,
