...
-spec check_types_in_ast(e_ast(), var_type_map(), {fn_type_map(), struct_type_map()}) ->
                            ok.
check_types_in_ast([#function{var_type_map = VarTypes,
                              stmts = Exprs,
                              type = FnType}
                    | Rest],
                   GlobalVarTypes,
...
check_types_in_ast([#struct{name = Name,
    ...
    check_types_in_struct_fields(InitFieldNames,
                                 FieldTypes,
                                 FieldDefaults,
                                 Name,
...
-spec check_type_in_ast_nodes([e_expr()],
                              var_type_map(),
...
type_of_ast_node(#op2_expr{operator = assign,
                           operand1 = Op1,
                           operand2 = Op2,
                           line = Line},
                 {_, _, StructMap, _} = Ctx) ->
    ...
    case compare_type(Op1Type, Op2Type) of
        ...
        false ->
            throw({Line,
                   e_util:fmt("type mismatch in \"~s = ~s\"",
                              [type_to_str(Op1Type), type_to_str(Op2Type)])})
    end;
type_of_ast_node(#op2_expr{operator = '.',
                           operand1 = Op1,
                           operand2 = Op2,
                           line = Line},
                 {_, _, StructMap, _} = Ctx) ->
...
type_of_ast_node(#op2_expr{operator = Operator,
                           operand1 = Op1,
                           operand2 = Op2,
                           line = Line},
                 Ctx)
...
type_of_ast_node(#op1_expr{operator = '^',
                           operand = Operand,
                           line = Line},
                 Ctx) ->
...
type_of_ast_node(#call_expr{fn = FunExpr,
                            args = Args,
                            line = Line},
                 Ctx) ->
...
type_of_ast_node(#if_stmt{condi = Condi,
                          then = Then,
                          else = Else,
                          line = Line},
                 Ctx) ->
...
type_of_ast_node(#while_stmt{condi = Condi,
                             stmts = Exprs,
                             line = Line},
                 Ctx) ->
    type_of_ast_node(Condi, Ctx),
...
type_of_ast_node(#return_stmt{expr = Expr, line = Line}, {_, _, _, FnRetType} = Ctx) ->
    ...
    case compare_type(RealRet, FnRetType) of
        ...
        false ->
            throw({Line,
                   e_util:fmt("ret type should be (~s), not (~s)",
                              [type_to_str(FnRetType), type_to_str(RealRet)])})
    end;
type_of_ast_node(#var_ref{name = Name, line = Line},
...
type_of_ast_node(#array_init_expr{elements = Elements, line = Line}, Ctx) ->
    ...
    case are_same_type(ElementTypes) of
        true ->
            #array_type{elem_type = hd(ElementTypes),
                        length = length(ElementTypes),
                        line = Line};
        false ->
            throw({Line,
                   e_util:fmt("array init type conflict: {~s}", [join_types_to_str(ElementTypes)])})
    end;
type_of_ast_node(#struct_init_expr{name = Name,
                                   field_names = FieldNames,
...
type_of_ast_node(#goto_label{line = Line}, _) ->
    e_util:void_type(Line);
type_of_ast_node(#type_convert{expr = Expr,
                               type = Type,
                               line = Line},
                 Ctx) ->
...
-spec check_types_in_struct_fields([#var_ref{}],
                                   var_type_map(),
                                   #{atom() := any()},
                                   atom(),
                                   context()) ->
...
check_types_in_struct_fields(FieldNames, FieldTypes, ValMap, StructName, Ctx) ->
    lists:foreach(fun(V) -> check_struct_field(V, FieldTypes, ValMap, StructName, Ctx) end,
                  FieldNames).

-spec check_struct_field(#var_ref{},
                         var_type_map(),
                         #{atom() := any()},
                         atom(),
                         context()) ->
                            ok.
check_struct_field(#var_ref{name = FieldName, line = Line},
                   FieldTypes,
                   ValMap,
                   StructName,
                   {_, _, StructMap, _} = Ctx) ->
...
-spec type_of_struct_field(e_type(), #var_ref{}, struct_type_map(), integer()) ->
                              e_type().
type_of_struct_field(#basic_type{class = struct,
                                 tag = Name,
                                 p_depth = 0},
                     #var_ref{name = FieldName},
                     StructMap,
                     Line) ->
    case maps:find(Name, StructMap) of
        {ok, #struct{field_type_map = FieldTypes}} ->
...
compare_type(#array_type{elem_type = E1, length = L1},
             #array_type{elem_type = E2, length = L2}) ->
    compare_type(E1, E2) and (L1 =:= L2);
compare_type(#basic_type{class = integer, p_depth = 0},
             #basic_type{class = integer, p_depth = 0}) ->
    true;
compare_type(#basic_type{class = C,
                         tag = T,
                         p_depth = P},
             #basic_type{class = C,
                         tag = T,
...
is_pointer_and_integer_ordered(#basic_type{p_depth = PDepth} = Type,
                               #basic_type{class = integer, p_depth = 0})
    when PDepth > 0 ->
    {true, Type};
...
is_pointer_and_integer(#basic_type{p_depth = PDepth} = Type,
                       #basic_type{class = integer, p_depth = 0})
    when PDepth > 0 ->
    {true, Type};
is_pointer_and_integer(#basic_type{class = integer, p_depth = 0},
...
are_both_integers(#basic_type{p_depth = 0, class = integer},
                  #basic_type{p_depth = 0, class = integer}) ->
    true;
are_both_integers(_, _) ->
    false.

-spec are_both_floats(e_type(), e_type()) -> boolean().
are_both_floats(#basic_type{p_depth = 0, class = float},
                #basic_type{p_depth = 0, class = float}) ->
    true;
are_both_floats(_, _) ->
    false.
...
type_error_of_op2(Operator, TypeofOp1, TypeofOp2) ->
    e_util:fmt("type error in \"~s ~s ~s\"",
               [type_to_str(TypeofOp1), Operator, type_to_str(TypeofOp2)]).
...
check_type(#basic_type{class = struct,
                       tag = Tag,
                       line = Line},
           StructMap) ->
