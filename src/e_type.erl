...
check_types_in_ast(
	...
	InitFieldNames = e_util:filter_var_refs_in_map(
		FieldNames, FieldDefaults
	),
	check_types_in_struct_fields(
		InitFieldNames,
		FieldTypes,
		FieldDefaults,
		Name,
...
type_of_node(
	...
	case maps:find(Name, StructMap) of
		{ok, #struct{field_type_map = FieldTypes}} ->
			check_types_in_struct_fields(
				FieldNames, FieldTypes, FieldValues, Name, Ctx
			),
			#basic_type{
				class = struct,
				tag = Name,
...
check_types_in_struct_fields(FieldNames, FieldTypes, ValMap, StructName, Ctx) ->
	lists:foreach(
		fun (V) ->
			check_struct_field(
				V, FieldTypes, ValMap, StructName, Ctx
			)
		end,
		FieldNames
	).
...
-spec type_of_struct_field(e_type(), #var_ref{}, struct_type_map(), integer())
	-> e_type().
type_of_struct_field(
	#basic_type{class = struct, tag = Name, p_depth = 0},
...
compare_type(
	#fn_type{params = P1, ret = R1},
	#fn_type{params = P2, ret = R2}
) ->
	compare_types(P1, P2) and compare_type(R1, R2);
compare_type(
	#array_type{elem_type = E1, length = L1},
...
compare_type(
	...
	true;
compare_type(
	#basic_type{class = C, tag = T, p_depth = P},
	#basic_type{class = C, tag = T, p_depth = P}
) ->
	true;
compare_type(_, _) ->
...
-spec is_pointer_and_integer_ordered(e_type(), e_type())
	-> {true, e_type()} | false.
...
-spec are_both_number_of_same_type(e_type(), e_type())
	-> {true, e_type()} | false.
...
type_to_str(#fn_type{params = Params, ret = RetType}) ->
	io_lib:format(
		"fun (~s): ~s",
		[join_types_to_str(Params), type_to_str(RetType)]
	);
