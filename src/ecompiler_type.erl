...
-export([checktype_ast/3, checktype_exprs/3,
	 typeof_expr/2]).

-import(ecompiler_util,
	[assert/2, expr2str/1, filter_varref_inmaps/2,
	 flat_format/2, void_type/1]).
...
checktype_ast([#function{var_types = VarTypes,
    ...
    Ctx = {CurrentVars, FunctionMap, StructMap,
	   Fntype#fun_type.ret},
...
typeof_expr(#op2{operator = '.', op1 = Op1, op2 = Op2,
		 line = Line},
	    {_, _, StructMap, _} = Ctx) ->
    typeof_structfield(typeof_expr(Op1, Ctx), Op2,
		       StructMap, Line);
...
typeof_expr(#op1{operator = '@', operand = Operand,
    ...
    case Operand of
	  ...
	  T = typeof_structfield(typeof_expr(Op1, Ctx), Op2,
				 StructMap, Line),
	  incr_pdepth(T, Line);
      #varref{} ->
...
typeof_expr(#varref{name = Name, line = Line},
    ...
    Type = case maps:find(Name, VarTypes) of
		 ...
		 case maps:find(Name, FunctionMap) of
		   error ->
		       throw({Line,
			      flat_format("variable ~s is undefined", [Name])});
		   {ok, T} -> T
		 end;
	     {ok, T} -> T
	   end,
...
incr_pdepth(#array_type{elemtype = #basic_type{} = T},
	    OpLine) ->
...
check_structfield(#varref{name = FieldName,
    ...
    ExpectedType = get_field_type(FieldName, FieldTypes,
				  StructName, Line),
    ...
    case compare_type(ExpectedType, GivenType) of
	  ...
	  throw({Line,
		 flat_format("~s.~s type error: ~s = ~s",
			     [StructName, FieldName, fmt_type(ExpectedType),
			      fmt_type(GivenType)])});
      _ -> ok
    end.
...
compare_type(#basic_type{class = integer, pdepth = 0},
    ...
    true;
compare_type(#basic_type{class = C, tag = T,
			 pdepth = P},
...
fmt_type(#fun_type{params = Params, ret = Rettype}) ->
    io_lib:format("fun(~s): ~s",
		  [fmt_types_join(Params), fmt_type(Rettype)]);
