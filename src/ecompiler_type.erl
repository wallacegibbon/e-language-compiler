...
typeof_expr(#varref{name=Name,line=Line}, {VarTypes,FunctionMap,StructMap,_}) ->
...
typeof_expr(#array_init{elements=Elements,line=Line}, Ctx) ->
	...
	true ->
	    #array_type{elemtype=hd(ElementTypes),len=length(ElementTypes),
			line=Line};
	_ ->
	    throw({Line,flat_format("array init type conflict: {~s}",
...
args_errorinfo(FnParamTypes, ArgsTypes) ->
    flat_format("args should be (~s), not (~s)",
		[fmt_types_join(FnParamTypes),fmt_types_join(ArgsTypes)]).
...
typeof_structfield(T, _, _, Line) ->
    throw({Line,flat_format("op1 for \".\" is not struct ~s", [fmt_type(T)])}).
...
checktype_type(#basic_type{class=struct,tag=Tag,line=Line}, StructMap) ->
	...
	error ->
	    throw({Line,flat_format("struct ~s is not found", [Tag])});
	{ok,_} ->
	    ok
    end;
