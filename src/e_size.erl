...
fill_struct_info(AST, {_, PointerWidth} = Ctx) ->
	...
	AST1 = lists:map(
		fun (E) -> fill_struct_size(E, Ctx) end,
		AST
	),
	{_, StructMap1} = e_util:make_function_and_struct_map_from_ast(AST1),
	lists:map(
		fun
		(E) -> fill_struct_offsets(E, {StructMap1, PointerWidth})
		end,
		AST1
	).
...
offset_of_struct(
	#struct{field_names = Names, field_type_map = FieldTypes},
	Ctx
) ->
