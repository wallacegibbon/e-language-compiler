...
typeOfExpression(#op2{operator = Operator, op1 = Operand1, op2 = Operand2, line = Line}, Ctx) when Operator =:= '*'; Operator =:= '/' ->
    ...
    case prvAreBothNumberSameType(TypeofOp1, TypeofOp2) of
        {true, T} ->
            T;
        false ->
            throw({Line, prvTypeErrorOfOp2(Operator, TypeofOp1, TypeofOp2)})
    end;
...
prvAreBothIntegers(#basic_type{pdepth = 0, class = integer}, #basic_type{pdepth = 0, class = integer}) ->
    true;
prvAreBothIntegers(_, _) ->
    false.

-spec prvAreBothFloats(eType(), eType()) -> boolean().
prvAreBothFloats(#basic_type{pdepth = 0, class = float}, #basic_type{pdepth = 0, class = float}) ->
    true;
prvAreBothFloats(_, _) ->
    false.
