...
fix_function_for_c(#function{} = F, {FnTypeMap, StructMap, GlobalVars}) ->
  ...
  F#function{stmts =
               fix_exprs_for_c(Exprs, {FnTypeMap, StructMap, maps:merge(GlobalVars, VarTypes)})};
...
fix_expr_for_c(#e_expr{tag = '@'} = E, {FnTypeMap, StructMap, VarTypes} = Ctx) ->
  ...
  case e_type:type_of_node(Operand, {VarTypes, FnTypeMap, StructMap, #basic_type{}}) of
    #array_type{} ->
      #e_expr{tag = '.',
              data = [fix_expr_for_c(Operand, Ctx), #var_ref{name = value, line = Line}]};
    _ ->
      E
  end;
...
expr_to_str(#e_expr{tag = Tag, data = [Op1, Op2]}, EndChar) ->
  io_lib:format("(~s ~s ~s)~c",
                [expr_to_str(Op1, $\s), translate_op(Tag), expr_to_str(Op2, $\s), EndChar]);
