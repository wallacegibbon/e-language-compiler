...
fix_function_for_c(#function{stmts = Exprs, var_type_map = VarTypes} = F,
                   {FnTypeMap, StructMap, GlobalVars}) ->
    F#function{stmts =
                   fix_exprs_for_c(Exprs,
...
fix_expr_for_c(#op1_expr{operator = '@',
                         operand = Operand,
                         line = Line} =
                   E,
...
fix_expr_for_c(#op2_expr{operand1 = Op1, operand2 = Op2} = E, Ctx) ->
    E#op2_expr{operand1 = fix_expr_for_c(Op1, Ctx), operand2 = fix_expr_for_c(Op2, Ctx)};
...
statements_to_str([#function{name = Name,
    ...
    Declars =
        function_declaration_to_str(Name, function_params_to_str(PureParams), FnType#fn_type.ret),
    Exprs2 =
        case Name =:= main of
            true ->
                InitCode ++ Exprs;
...
statements_to_str([#struct{name = Name,
...
return_type_to_str(#fn_type{params = Params, ret = RetType}, NameParams) ->
    NewNameParams =
        io_lib:format("~s(~s)", [NameParams, function_params_to_str_no_name(Params)]),
...
type_to_c_str(#basic_type{class = Class,
                          tag = Tag,
                          p_depth = Depth},
              VarName)
    when Depth > 0 ->
    io_lib:format("~s~s ~s",
                  [type_tag_to_str(Class, Tag), lists:duplicate(Depth, "*"), VarName]);
type_to_c_str(#basic_type{class = Class,
                          tag = Tag,
                          p_depth = 0},
              VarName) ->
...
type_to_c_str(#fn_type{params = Params, ret = RetType}, VarName) ->
    NameParams =
        io_lib:format("(*~s)(~s)", [VarName, function_params_to_str_no_name(Params)]),
...
expr_to_str(#if_stmt{condi = Condi,
                     then = Then,
                     else = Else},
            _) ->
    io_lib:format("if (~s) {\n~s\n} else {\n~s}",
                  [expr_to_str(Condi, $\s), exprs_to_str(Then), exprs_to_str(Else)]);
expr_to_str(#while_stmt{condi = Condi, stmts = Exprs}, _) ->
    io_lib:format("while (~s) {\n~s\n}\n", [expr_to_str(Condi, $\s), exprs_to_str(Exprs)]);
expr_to_str(#op2_expr{operator = '::',
                      operand1 = #var_ref{name = c},
                      operand2 = Op2},
            EndChar) ->
    expr_to_str(Op2, EndChar);
expr_to_str(#op2_expr{operator = Operator,
                      operand1 = Op1,
                      operand2 = Op2},
            EndChar) ->
    io_lib:format("(~s ~s ~s)~c",
                  [expr_to_str(Op1, $\s), translate_op(Operator), expr_to_str(Op2, $\s), EndChar]);
