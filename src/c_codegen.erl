...
statements_to_str(
	...
	PureParams = map_to_kv_list(
		ParamNameAtoms,
		maps:with(ParamNameAtoms, VarTypes)
	),
	PureVars = maps:without(ParamNameAtoms, VarTypes),
	Declars = function_declaration_to_str(
		Name,
		function_params_to_str(PureParams),
		FnType#fn_type.ret
	),
	Exprs2 =
		case Name =:= main of
...
statements_to_str(
	...
	S = io_lib:format(
		"struct ~s {~n~s~n};~n~n",
		[Name, var_list_to_str(FieldList)]
	),
...
function_params_to_str_no_name(Types) ->
	lists:join(
		",",
		lists:map(fun (T) -> type_to_c_str(T, "") end, Types)
	).
...
fix_special_chars(String) ->
	lists:map(
		fun (C) -> maps:get(C, ?SPECIAL_CHARACTER_MAP, C) end,
		String
	).
