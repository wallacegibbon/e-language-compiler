...
fix_function_for_c(#function{stmts = Exprs, var_type_map = VarTypes} = F,  {FnTypeMap, StructMap, GlobalVars}) ->
...
fix_expr_for_c(#op1_expr{operator = '@', operand = Operand, line = Line} = E, {FnTypeMap, StructMap, VarTypes} = Ctx) ->
    case e_type:type_of_node(Operand, {VarTypes, FnTypeMap, StructMap, #{}}) of
        #array_type{} ->
...
statements_to_str([#function{name = Name, param_names = ParamNames, type = FnType, var_type_map = VarTypes, stmts = Exprs} | Rest],
...
statements_to_str([#struct{name = Name, field_type_map = FieldTypes, field_names = FieldNames} | Rest],
                  InitCode, StmtStrs, FnDeclars) ->
...
expr_to_str(#op2_expr{operator = '::', operand1 = #var_ref{name = c}, operand2 = Op2}, EndChar) ->
    expr_to_str(Op2, EndChar);
expr_to_str(#op2_expr{operator = Operator, operand1 = Op1, operand2 = Op2}, EndChar) ->
    io_lib:format("(~s ~s ~s)~c", [expr_to_str(Op1, $\s), translate_op(Operator), expr_to_str(Op2, $\s), EndChar]);
