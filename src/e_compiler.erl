...
parse_file(Filename) ->
	...
	{ok, RawContent}		-> parse_string(RawContent);
	{error, enoent}			-> throw("file not found");
...
parse_string(RawContent) ->
	...
	{error, ErrorInfo, Line}	-> throw({Line, ErrorInfo})
	end.
...
parse_tokens(Tokens) ->
	...
	{ok, AST}			-> AST;
	{error, {Line, _, ErrorInfo}}	-> throw({Line, ErrorInfo})
	end.
