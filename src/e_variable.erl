...
fetch_variables([#var_def{} = Hd | Rest], NewAST, {VarTypes, InitCode, CollectInitCode}) ->
	#var_def{name = Name, type = Type, line = Line, init_value = InitialValue} = Hd,
...
fetch_variables([#function_raw{} = Hd | Rest], NewAST, {GlobalVars, _, _} = Ctx) ->
	#function_raw{name = Name, ret_type = Ret, params = Params, stmts = Stmts, line = Line} = Hd,
	...
	Function = #function{name = Name, var_type_map = FnVarTypes, stmts = NewExprs, param_names = var_defs_to_refs(Params), line = Line, type = FnType},
...
append_to_ast(AST, VarName, InitialValue, Line) when InitialValue =/= none ->
	[#e_expr{tag = '=', data = [#var_ref{name = VarName, line = Line}, InitialValue], line = Line} | AST];
