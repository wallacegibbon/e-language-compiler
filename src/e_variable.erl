...
prepare_struct_init_expr([#function_raw{statements = Expressions} = F | Rest]) ->
...
fix_struct_init(#operator_expression1{operand = Operand} = O) ->
    O#operator_expression1{operand = fix_struct_init(Operand)};
fix_struct_init(Any) ->
    Any.
...
-spec struct_init_to_map([#operator_expression2{}], [#variable_reference{}], #{atom() := e_expr()}) -> {[#variable_reference{}], #{atom() := e_expr()}}.
struct_init_to_map([#operator_expression2{operator = assign, operand1 = #variable_reference{name = Field} = Operand1, operand2 = Val} | Rest], FieldNames, ExprMap) ->
...
appendToAST(AST, VariableName, InitialValue, Line) when InitialValue =/= none ->
    [#operator_expression2{operator = assign, operand1 = #variable_reference{name = VariableName, line = Line}, operand2 = InitialValue, line = Line} | AST];
...
-spec get_values_by_defs([#variable_definition{}], #{atom() => any()}) -> [any()].
get_values_by_defs(DefList, Map) ->
    e_util:get_values_by_keys(e_util:names_of_var_defs(DefList), Map).

-spec var_defs_to_refs([#variable_definition{}]) -> [#variable_reference{}].
var_defs_to_refs(VariableDefinitions) ->
    lists:map(fun (#variable_definition{name = N, line = Line}) -> #variable_reference{name = N, line = Line} end, VariableDefinitions).
