...
{CharQuote}({CharUnescapedChar}|{CharEscapedChar}){CharQuote} : {token, {integer, TokenLine, fixCharacter(dropQuotes(TokenChars))}}.
0x{HexDigit}+ : {token, {integer, TokenLine, stringToInteger(TokenChars, 16)}}.
0o{OctallDigit}+ : {token, {integer, TokenLine, stringToInteger(TokenChars, 8)}}.
...
struct|end|fun|return|if|then|elif|else|while|do|goto|sizeof|as : {token, {list_to_atom(TokenChars), TokenLine}}.
